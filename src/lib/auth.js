...
const ERROR_MESSAGES = {
  ...
  account_already_logged_in: '该账号已登录。请先退出当前登录后再尝试。',
...
/* ---------- auth actions ---------- */
export async function register({ inviteCode, username, password, displayName, tournamentRole, gender, avatarUrl }) {
...
export async function logout() {
  ...
  if (token) {
    try {
      ...
      // Best-effort: the token is already cleared client-side either way,
      // and the server-side session will time out on its own (Section 15)
      // even if this call didn't reach the server.
    }
  }
}
