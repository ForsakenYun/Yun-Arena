...
-- teams, captainCandidates, pool, pickIndex, roundOrders, draftHistory}
...
create table if not exists public.tournament_draft_state (
  ...
  updated_at  timestamptz not null default now(),
  updated_by  uuid references public.accounts(id) on delete set null,
  constraint tournament_draft_state_singleton check (id)
);

comment on table public.tournament_draft_state is
  'Singleton row. Public read, written only through sync_draft_state(), Admin/Developer-only. The persisted source of truth for draft progress -- read back by DraftArenaPage (resuming an in-progress draft) and by the read-only Spectator Page (rendering whatever was last saved, independent of live connections). Absence of this row means no draft is currently in progress (or it already reached Final Matchups / ended).';
...
-- Generates a small, original, self-contained placeholder avatar as an
-- inline SVG data URI -- no network call, no external host, nothing to
-- rot or go offline later. Purely cosmetic/stateless (no DB access), so
-- it's deliberately NOT in the RPC grant list below like the other
-- "_"-prefixed helpers (_require_role, _current_session_account) --
-- there's nothing here worth restricting, but no reason to expose it
-- either.
--
-- Ten original "badge icon" designs (mountain, river, dragon, space,
-- flame, tree, crystal, moon, compass, wave), each in 4 precomputed
-- colorways (40 total), fully baked ahead of time in v_svgs below --
-- picked with a straight ((p_seed-1) % 40) + 1 lookup, no per-call
-- computation. Two things were deliberately kept OUT of the generated
-- markup after the Draft Arena's "Card Slide" assignment animation
-- (DraftArena.jsx, runFlight()) turned out to lag once every temp
-- account had an avatar -- both were genuine, measured causes, not
-- just theoretical:
--   1. An earlier revision shared one <defs> block (all 10 themes'
--      gradients, always, regardless of which theme was shown) across
--      every single avatar -- ~6KB of unused gradient defs per avatar.
--      Each entry below instead embeds only the 2-4 gradients its own
--      theme actually uses, cutting average size from ~7-8KB to ~1.3KB.
--   2. Colorway variety used to come from a live SVG filter
--      (feColorMatrix type="hueRotate") wrapping the icon, plus a
--      feDropShadow (Gaussian blur) for the shadow -- both are exactly
--      the kind of filter primitive that forces a full software
--      re-rasterization on every animation frame, which is very
--      noticeable on a freshly-cloned <img> being animated across the
--      screen. The 4 colorways are now precomputed literal colors
--      (same math as feColorMatrix's hueRotate, applied once in SQL
--      text at write time below) and the drop shadow is a flat,
--      unblurred duplicate shape nudged down-right -- visually close,
--      costs nothing to paint. The result has zero <filter> elements.
create or replace function public._temp_avatar_svg(p_seed int)
returns text
language plpgsql
as $BODY$
declare
  v_svgs constant text[] := array[
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgMtn" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#cfe8fb"/><stop offset="100%" stop-color="#8fc2ec"/></radialGradient><linearGradient id="sunG" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#ffe58a"/><stop offset="100%" stop-color="#ffb648"/></linearGradient><linearGradient id="peak1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#9db3d6"/><stop offset="100%" stop-color="#5c7292"/></linearGradient><linearGradient id="peak2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#b9c8e0"/><stop offset="100%" stop-color="#7a8fae"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgMtn)"/><g transform="translate(4,6)" opacity="0.22"><circle cx="150" cy="52" r="20" fill="#1a1a1a"/></g><g transform="translate(4,6)" opacity="0.22"><path d="M15,155 L78,65 L118,112 L155,50 L188,155 Z" fill="#1a1a1a"/></g><circle cx="150" cy="52" r="20" fill="url(#sunG)"/><path d="M15,155 L78,65 L118,112 L155,50 L188,155 Z" fill="url(#peak2)"/><path d="M155,50 L172,78 L138,78 Z" fill="#ffffff"/><path d="M15,155 L78,65 L108,105 L55,155 Z" fill="url(#peak1)"/><path d="M78,65 L96,92 L60,92 Z" fill="#ffffff"/><ellipse cx="95" cy="60" rx="40" ry="14" fill="#ffffff" opacity="0.25" transform="rotate(-25 95 60)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgRiv" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#e8fbf7"/><stop offset="100%" stop-color="#bdeee3"/></radialGradient><linearGradient id="w1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#2f9c92"/><stop offset="100%" stop-color="#1e6f68"/></linearGradient><linearGradient id="w2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#57c2b4"/><stop offset="100%" stop-color="#2f9c92"/></linearGradient><linearGradient id="w3" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#a6e8dc"/><stop offset="100%" stop-color="#68d3c1"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgRiv)"/><g transform="translate(4,6)" opacity="0.22"><path d="M0,75 Q40,50 80,75 T160,75 T200,75 V102 Q150,80 100,102 T0,102 Z" fill="#1a1a1a"/></g><path d="M0,75 Q40,50 80,75 T160,75 T200,75 V102 Q150,80 100,102 T0,102 Z" fill="url(#w1)"/><path d="M0,112 Q40,90 80,112 T160,112 T200,112 V138 Q150,116 100,138 T0,138 Z" fill="url(#w2)"/><path d="M0,148 Q40,128 80,148 T160,148 T200,148 V172 Q150,150 100,172 T0,172 Z" fill="url(#w3)"/><ellipse cx="60" cy="60" rx="45" ry="10" fill="#ffffff" opacity="0.3" transform="rotate(-15 60 60)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgDrg" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#f3e9fb"/><stop offset="100%" stop-color="#d9c2f0"/></radialGradient><linearGradient id="drgBody" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#a98ee0"/><stop offset="100%" stop-color="#6f4fb5"/></linearGradient><linearGradient id="drgWing" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#c6aef0"/><stop offset="100%" stop-color="#8a6fd6"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgDrg)"/><g transform="translate(4,6)" opacity="0.22"><path d="M60,150 Q40,120 55,95 Q45,80 60,72 Q58,55 78,52 Q80,35 100,42 Q112,25 130,38 Q150,30 155,50 Q170,52 165,70 Q180,75 170,92 Q178,105 160,108 L165,130 L145,118 L142,140 L122,125 L115,148 L98,128 L82,148 L75,125 Z" fill="#1a1a1a"/></g><path d="M60,150 Q40,120 55,95 Q45,80 60,72 Q58,55 78,52 Q80,35 100,42 Q112,25 130,38 Q150,30 155,50 Q170,52 165,70 Q180,75 170,92 Q178,105 160,108 L165,130 L145,118 L142,140 L122,125 L115,148 L98,128 L82,148 L75,125 Z" fill="url(#drgBody)"/><path d="M130,38 Q145,15 165,25 Q155,40 155,50 Z" fill="url(#drgWing)"/><path d="M100,42 Q108,20 125,28 Q118,42 112,50 Z" fill="url(#drgWing)"/><circle cx="152" cy="58" r="4.5" fill="#000000"/><circle cx="150.5" cy="56" r="1.5" fill="#ffffff"/><path d="M168,68 Q180,66 178,78 Q170,76 168,68 Z" fill="#e0a24d"/><ellipse cx="95" cy="55" rx="40" ry="12" fill="#ffffff" opacity="0.25" transform="rotate(-20 95 55)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgSpc" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#2a3568"/><stop offset="100%" stop-color="#141a33"/></radialGradient><radialGradient id="moonG" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#fdf6e9"/><stop offset="100%" stop-color="#e3d3ae"/></radialGradient><radialGradient id="planetG" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#f3c76a"/><stop offset="100%" stop-color="#c98f2f"/></radialGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgSpc)"/><g transform="translate(4,6)" opacity="0.22"><circle cx="150" cy="58" r="26" fill="#000000"/></g><circle cx="150" cy="58" r="26" fill="url(#moonG)"/><circle cx="140" cy="50" r="22" fill="#1f2650"/><circle cx="45" cy="45" r="2.5" fill="#ffffff"/><circle cx="72" cy="88" r="2" fill="#ffffff"/><circle cx="28" cy="105" r="2.2" fill="#ffffff"/><circle cx="105" cy="135" r="1.8" fill="#ffffff"/><circle cx="165" cy="145" r="2.2" fill="#ffffff"/><ellipse cx="58" cy="158" rx="62" ry="18" fill="none" stroke="#e0b96a" stroke-width="2.5" opacity="0.6"/><g transform="translate(4,6)" opacity="0.22"><circle cx="58" cy="158" r="13" fill="#000000"/></g><circle cx="58" cy="158" r="13" fill="url(#planetG)"/><ellipse cx="148" cy="50" rx="10" ry="6" fill="#ffffff" opacity="0.4" transform="rotate(-20 148 50)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgFlm" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#fff0e0"/><stop offset="100%" stop-color="#ffd9b8"/></radialGradient><linearGradient id="flmOut" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#ff8a3d"/><stop offset="100%" stop-color="#d4501f"/></linearGradient><linearGradient id="flmIn" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#ffd166"/><stop offset="100%" stop-color="#ff9f3d"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgFlm)"/><g transform="translate(4,6)" opacity="0.22"><path d="M100,172 C58,172 52,128 74,96 C80,118 96,118 90,90 C112,102 132,128 120,150 C142,138 140,106 124,86 C150,102 156,150 130,166 C162,154 150,106 133,80 C118,54 88,54 88,86 C72,74 68,44 90,24 C58,46 36,88 53,128 C36,140 40,168 100,172 Z" fill="#1a1a1a"/></g><path d="M100,172 C58,172 52,128 74,96 C80,118 96,118 90,90 C112,102 132,128 120,150 C142,138 140,106 124,86 C150,102 156,150 130,166 C162,154 150,106 133,80 C118,54 88,54 88,86 C72,74 68,44 90,24 C58,46 36,88 53,128 C36,140 40,168 100,172 Z" fill="url(#flmOut)"/><path d="M96,160 C79,154 76,132 90,114 C92,128 101,128 97,112 C112,120 121,138 109,152 C124,144 121,120 109,106 C121,116 126,142 109,154 C96,160 96,160 96,160 Z" fill="url(#flmIn)"/><ellipse cx="85" cy="60" rx="20" ry="10" fill="#ffffff" opacity="0.3" transform="rotate(-20 85 60)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgTree" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#eaf8e6"/><stop offset="100%" stop-color="#c7ecc0"/></radialGradient><linearGradient id="tr1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#57b073"/><stop offset="100%" stop-color="#2f7a48"/></linearGradient><linearGradient id="tr2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#69c085"/><stop offset="100%" stop-color="#3f9a5c"/></linearGradient><linearGradient id="tr3" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#7fd096"/><stop offset="100%" stop-color="#4fae6c"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgTree)"/><rect x="93" y="150" width="14" height="32" rx="4" fill="#8b6f5c"/><g transform="translate(4,6)" opacity="0.22"><path d="M100,42 L138,105 H62 Z" fill="#1a1a1a"/></g><path d="M100,42 L138,105 H62 Z" fill="url(#tr1)"/><path d="M100,74 L143,140 H57 Z" fill="url(#tr2)"/><path d="M100,110 L148,178 H52 Z" fill="url(#tr3)"/><ellipse cx="95" cy="90" rx="30" ry="10" fill="#ffffff" opacity="0.25" transform="rotate(-25 95 90)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgCry" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#f2e9fb"/><stop offset="100%" stop-color="#dcc4f0"/></radialGradient><linearGradient id="cA" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#d3c2ea"/><stop offset="100%" stop-color="#b39ddb"/></linearGradient><linearGradient id="cB" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#9b80c9"/><stop offset="100%" stop-color="#6f4fb5"/></linearGradient><linearGradient id="cC" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#c0a8e6"/><stop offset="100%" stop-color="#8a6fd6"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgCry)"/><g transform="translate(4,6)" opacity="0.22"><path d="M100,28 L148,76 L120,172 L80,172 L52,76 Z" fill="#1a1a1a"/></g><path d="M100,28 L148,76 L120,172 L80,172 L52,76 Z" fill="url(#cA)"/><path d="M100,28 L148,76 L100,92 Z" fill="#e6dcf5"/><path d="M100,28 L52,76 L100,92 Z" fill="url(#cB)"/><path d="M52,76 L100,92 L80,172 Z" fill="url(#cB)"/><path d="M148,76 L100,92 L120,172 Z" fill="url(#cC)"/><ellipse cx="95" cy="55" rx="18" ry="22" fill="#ffffff" opacity="0.4" transform="rotate(-10 95 55)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgMoon" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#262c56"/><stop offset="100%" stop-color="#141834"/></radialGradient><linearGradient id="moonG2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#fbeecb"/><stop offset="100%" stop-color="#e6cf94"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgMoon)"/><g transform="translate(4,6)" opacity="0.22"><path d="M132,38 A56,56 0 1 0 132,162 A46,46 0 1 1 132,38 Z" fill="#000000"/></g><path d="M132,38 A56,56 0 1 0 132,162 A46,46 0 1 1 132,38 Z" fill="url(#moonG2)"/><circle cx="56" cy="55" r="2.5" fill="#ffffff"/><circle cx="40" cy="92" r="2" fill="#ffffff"/><circle cx="62" cy="132" r="2.5" fill="#ffffff"/><circle cx="34" cy="152" r="1.8" fill="#ffffff"/><circle cx="90" cy="45" r="1.5" fill="#ffffff"/><ellipse cx="120" cy="55" rx="20" ry="12" fill="#ffffff" opacity="0.3" transform="rotate(-20 120 55)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgCmp" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#fdf6e3"/><stop offset="100%" stop-color="#f3e4b8"/></radialGradient><linearGradient id="cmpN" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#e0645f"/><stop offset="100%" stop-color="#b8342f"/></linearGradient><linearGradient id="cmpS" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#94a8c9"/><stop offset="100%" stop-color="#67809f"/></linearGradient><radialGradient id="cmpRing" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#e6c877"/><stop offset="100%" stop-color="#b98d3a"/></radialGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgCmp)"/><g transform="translate(4,6)" opacity="0.22"><circle cx="100" cy="100" r="68" fill="none" stroke="#1a1a1a" stroke-width="6"/></g><circle cx="100" cy="100" r="68" fill="none" stroke="url(#cmpRing)" stroke-width="6"/><circle cx="100" cy="100" r="58" fill="#fffaf0" opacity="0.5"/><path d="M100,42 L114,95 L100,112 L86,95 Z" fill="url(#cmpN)"/><path d="M100,158 L114,105 L100,88 L86,105 Z" fill="url(#cmpS)"/><path d="M42,100 L95,86 L112,100 L95,114 Z" fill="url(#cmpS)"/><path d="M158,100 L105,86 L88,100 L105,114 Z" fill="url(#cmpS)"/><circle cx="100" cy="100" r="7" fill="#c9a34e"/><ellipse cx="75" cy="65" rx="30" ry="12" fill="#ffffff" opacity="0.35" transform="rotate(-20 75 65)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgWav" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#e3f4fb"/><stop offset="100%" stop-color="#bfe6f5"/></radialGradient><linearGradient id="wv1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#2f7fa8"/><stop offset="100%" stop-color="#1f5c80"/></linearGradient><linearGradient id="wv2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#4a9dc4"/><stop offset="100%" stop-color="#2f7fa8"/></linearGradient><linearGradient id="wv3" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#8fd0e6"/><stop offset="100%" stop-color="#5db3d4"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgWav)"/><g transform="translate(4,6)" opacity="0.22"><path d="M0,85 Q40,55 80,85 T160,85 T200,85 V200 H0 Z" fill="#1a1a1a"/></g><path d="M0,85 Q40,55 80,85 T160,85 T200,85 V200 H0 Z" fill="url(#wv1)"/><path d="M0,118 Q40,92 80,118 T160,118 T200,118 V200 H0 Z" fill="url(#wv2)"/><path d="M0,150 Q40,128 80,150 T160,150 T200,150 V200 H0 Z" fill="url(#wv3)"/><ellipse cx="60" cy="55" rx="45" ry="12" fill="#ffffff" opacity="0.3" transform="rotate(-15 60 55)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgMtn" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#f9dbfb"/><stop offset="100%" stop-color="#e8a8ea"/></radialGradient><linearGradient id="sunG" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#8dffc1"/><stop offset="100%" stop-color="#4ee672"/></linearGradient><linearGradient id="peak1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#d4a4c8"/><stop offset="100%" stop-color="#906486"/></linearGradient><linearGradient id="peak2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#dfbed6"/><stop offset="100%" stop-color="#ac81a2"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgMtn)"/><g transform="translate(4,6)" opacity="0.22"><circle cx="150" cy="52" r="20" fill="#1a1a1a"/></g><g transform="translate(4,6)" opacity="0.22"><path d="M15,155 L78,65 L118,112 L155,50 L188,155 Z" fill="#1a1a1a"/></g><circle cx="150" cy="52" r="20" fill="url(#sunG)"/><path d="M15,155 L78,65 L118,112 L155,50 L188,155 Z" fill="url(#peak2)"/><path d="M155,50 L172,78 L138,78 Z" fill="#ffffff"/><path d="M15,155 L78,65 L108,105 L55,155 Z" fill="url(#peak1)"/><path d="M78,65 L96,92 L60,92 Z" fill="#ffffff"/><ellipse cx="95" cy="60" rx="40" ry="14" fill="#ffffff" opacity="0.25" transform="rotate(-25 95 60)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgRiv" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#f6f5ff"/><stop offset="100%" stop-color="#e0e0ff"/></radialGradient><linearGradient id="w1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#8b79da"/><stop offset="100%" stop-color="#62559d"/></linearGradient><linearGradient id="w2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#ada1fe"/><stop offset="100%" stop-color="#8b79da"/></linearGradient><linearGradient id="w3" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#d8d4ff"/><stop offset="100%" stop-color="#bab3ff"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgRiv)"/><g transform="translate(4,6)" opacity="0.22"><path d="M0,75 Q40,50 80,75 T160,75 T200,75 V102 Q150,80 100,102 T0,102 Z" fill="#1a1a1a"/></g><path d="M0,75 Q40,50 80,75 T160,75 T200,75 V102 Q150,80 100,102 T0,102 Z" fill="url(#w1)"/><path d="M0,112 Q40,90 80,112 T160,112 T200,112 V138 Q150,116 100,138 T0,138 Z" fill="url(#w2)"/><path d="M0,148 Q40,128 80,148 T160,148 T200,148 V172 Q150,150 100,172 T0,172 Z" fill="url(#w3)"/><ellipse cx="60" cy="60" rx="45" ry="10" fill="#ffffff" opacity="0.3" transform="rotate(-15 60 60)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgDrg" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#fce8e7"/><stop offset="100%" stop-color="#f1c0bf"/></radialGradient><linearGradient id="drgBody" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#e18590"/><stop offset="100%" stop-color="#b64453"/></linearGradient><linearGradient id="drgWing" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#f1a8af"/><stop offset="100%" stop-color="#d76276"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgDrg)"/><g transform="translate(4,6)" opacity="0.22"><path d="M60,150 Q40,120 55,95 Q45,80 60,72 Q58,55 78,52 Q80,35 100,42 Q112,25 130,38 Q150,30 155,50 Q170,52 165,70 Q180,75 170,92 Q178,105 160,108 L165,130 L145,118 L142,140 L122,125 L115,148 L98,128 L82,148 L75,125 Z" fill="#1a1a1a"/></g><path d="M60,150 Q40,120 55,95 Q45,80 60,72 Q58,55 78,52 Q80,35 100,42 Q112,25 130,38 Q150,30 155,50 Q170,52 165,70 Q180,75 170,92 Q178,105 160,108 L165,130 L145,118 L142,140 L122,125 L115,148 L98,128 L82,148 L75,125 Z" fill="url(#drgBody)"/><path d="M130,38 Q145,15 165,25 Q155,40 155,50 Z" fill="url(#drgWing)"/><path d="M100,42 Q108,20 125,28 Q118,42 112,50 Z" fill="url(#drgWing)"/><circle cx="152" cy="58" r="4.5" fill="#000000"/><circle cx="150.5" cy="56" r="1.5" fill="#ffffff"/><path d="M168,68 Q180,66 178,78 Q170,76 168,68 Z" fill="#52c96a"/><ellipse cx="95" cy="55" rx="40" ry="12" fill="#ffffff" opacity="0.25" transform="rotate(-20 95 55)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgSpc" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#672647"/><stop offset="100%" stop-color="#321323"/></radialGradient><radialGradient id="moonG" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#eafbef"/><stop offset="100%" stop-color="#afe0c1"/></radialGradient><radialGradient id="planetG" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#6eea98"/><stop offset="100%" stop-color="#34b757"/></radialGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgSpc)"/><g transform="translate(4,6)" opacity="0.22"><circle cx="150" cy="58" r="26" fill="#000000"/></g><circle cx="150" cy="58" r="26" fill="url(#moonG)"/><circle cx="140" cy="50" r="22" fill="#4f1b34"/><circle cx="45" cy="45" r="2.5" fill="#ffffff"/><circle cx="72" cy="88" r="2" fill="#ffffff"/><circle cx="28" cy="105" r="2.2" fill="#ffffff"/><circle cx="105" cy="135" r="1.8" fill="#ffffff"/><circle cx="165" cy="145" r="2.2" fill="#ffffff"/><ellipse cx="58" cy="158" rx="62" ry="18" fill="none" stroke="#6dd790" stroke-width="2.5" opacity="0.6"/><g transform="translate(4,6)" opacity="0.22"><circle cx="58" cy="158" r="13" fill="#000000"/></g><circle cx="58" cy="158" r="13" fill="url(#planetG)"/><ellipse cx="148" cy="50" rx="10" ry="6" fill="#ffffff" opacity="0.4" transform="rotate(-20 148 50)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgFlm" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#e1f9e4"/><stop offset="100%" stop-color="#bbedbb"/></radialGradient><linearGradient id="flmOut" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#46c234"/><stop offset="100%" stop-color="#298700"/></linearGradient><linearGradient id="flmIn" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#6af89e"/><stop offset="100%" stop-color="#45d550"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgFlm)"/><g transform="translate(4,6)" opacity="0.22"><path d="M100,172 C58,172 52,128 74,96 C80,118 96,118 90,90 C112,102 132,128 120,150 C142,138 140,106 124,86 C150,102 156,150 130,166 C162,154 150,106 133,80 C118,54 88,54 88,86 C72,74 68,44 90,24 C58,46 36,88 53,128 C36,140 40,168 100,172 Z" fill="#1a1a1a"/></g><path d="M100,172 C58,172 52,128 74,96 C80,118 96,118 90,90 C112,102 132,128 120,150 C142,138 140,106 124,86 C150,102 156,150 130,166 C162,154 150,106 133,80 C118,54 88,54 88,86 C72,74 68,44 90,24 C58,46 36,88 53,128 C36,140 40,168 100,172 Z" fill="url(#flmOut)"/><path d="M96,160 C79,154 76,132 90,114 C92,128 101,128 97,112 C112,120 121,138 109,152 C124,144 121,120 109,106 C121,116 126,142 109,154 C96,160 96,160 96,160 Z" fill="url(#flmIn)"/><ellipse cx="85" cy="60" rx="20" ry="10" fill="#ffffff" opacity="0.3" transform="rotate(-20 85 60)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgTree" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#e5f7fc"/><stop offset="100%" stop-color="#bee9f8"/></radialGradient><linearGradient id="tr1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#6d9fd7"/><stop offset="100%" stop-color="#436c9b"/></linearGradient><linearGradient id="tr2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#80afe6"/><stop offset="100%" stop-color="#5689c2"/></linearGradient><linearGradient id="tr3" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#91c1f3"/><stop offset="100%" stop-color="#669cd7"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgTree)"/><rect x="93" y="150" width="14" height="32" rx="4" fill="#5e7d5a"/><g transform="translate(4,6)" opacity="0.22"><path d="M100,42 L138,105 H62 Z" fill="#1a1a1a"/></g><path d="M100,42 L138,105 H62 Z" fill="url(#tr1)"/><path d="M100,74 L143,140 H57 Z" fill="url(#tr2)"/><path d="M100,110 L148,178 H52 Z" fill="url(#tr3)"/><ellipse cx="95" cy="90" rx="30" ry="10" fill="#ffffff" opacity="0.25" transform="rotate(-25 95 90)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgCry" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#fbe8e8"/><stop offset="100%" stop-color="#f1c3c0"/></radialGradient><linearGradient id="cA" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#ebbfc1"/><stop offset="100%" stop-color="#dc979e"/></linearGradient><linearGradient id="cB" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#ca7980"/><stop offset="100%" stop-color="#b64453"/></linearGradient><linearGradient id="cC" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#e7a3a8"/><stop offset="100%" stop-color="#d76276"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgCry)"/><g transform="translate(4,6)" opacity="0.22"><path d="M100,28 L148,76 L120,172 L80,172 L52,76 Z" fill="#1a1a1a"/></g><path d="M100,28 L148,76 L120,172 L80,172 L52,76 Z" fill="url(#cA)"/><path d="M100,28 L148,76 L100,92 Z" fill="#f5dadc"/><path d="M100,28 L52,76 L100,92 Z" fill="url(#cB)"/><path d="M52,76 L100,92 L80,172 Z" fill="url(#cB)"/><path d="M148,76 L100,92 L120,172 Z" fill="url(#cC)"/><ellipse cx="95" cy="55" rx="18" ry="22" fill="#ffffff" opacity="0.4" transform="rotate(-10 95 55)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgMoon" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#552139"/><stop offset="100%" stop-color="#331121"/></radialGradient><linearGradient id="moonG2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#ccfade"/><stop offset="100%" stop-color="#96e4b4"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgMoon)"/><g transform="translate(4,6)" opacity="0.22"><path d="M132,38 A56,56 0 1 0 132,162 A46,46 0 1 1 132,38 Z" fill="#000000"/></g><path d="M132,38 A56,56 0 1 0 132,162 A46,46 0 1 1 132,38 Z" fill="url(#moonG2)"/><circle cx="56" cy="55" r="2.5" fill="#ffffff"/><circle cx="40" cy="92" r="2" fill="#ffffff"/><circle cx="62" cy="132" r="2.5" fill="#ffffff"/><circle cx="34" cy="152" r="1.8" fill="#ffffff"/><circle cx="90" cy="45" r="1.5" fill="#ffffff"/><ellipse cx="120" cy="55" rx="20" ry="12" fill="#ffffff" opacity="0.3" transform="rotate(-20 120 55)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgCmp" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#e4fcee"/><stop offset="100%" stop-color="#b9f3d1"/></radialGradient><linearGradient id="cmpN" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#688f1a"/><stop offset="100%" stop-color="#386200"/></linearGradient><linearGradient id="cmpS" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#c79abb"/><stop offset="100%" stop-color="#9d7196"/></linearGradient><radialGradient id="cmpRing" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#7ae4a4"/><stop offset="100%" stop-color="#3eae60"/></radialGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgCmp)"/><g transform="translate(4,6)" opacity="0.22"><circle cx="100" cy="100" r="68" fill="none" stroke="#1a1a1a" stroke-width="6"/></g><circle cx="100" cy="100" r="68" fill="none" stroke="url(#cmpRing)" stroke-width="6"/><circle cx="100" cy="100" r="58" fill="#f0fef5" opacity="0.5"/><path d="M100,42 L114,95 L100,112 L86,95 Z" fill="url(#cmpN)"/><path d="M100,158 L114,105 L100,88 L86,105 Z" fill="url(#cmpS)"/><path d="M42,100 L95,86 L112,100 L95,114 Z" fill="url(#cmpS)"/><path d="M158,100 L105,86 L88,100 L105,114 Z" fill="url(#cmpS)"/><circle cx="100" cy="100" r="7" fill="#51c27a"/><ellipse cx="75" cy="65" rx="30" ry="12" fill="#ffffff" opacity="0.35" transform="rotate(-20 75 65)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgWav" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#faedff"/><stop offset="100%" stop-color="#f2d6ff"/></radialGradient><linearGradient id="wv1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#a25bb7"/><stop offset="100%" stop-color="#7b4088"/></linearGradient><linearGradient id="wv2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#be79d7"/><stop offset="100%" stop-color="#a25bb7"/></linearGradient><linearGradient id="wv3" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#e1b5fb"/><stop offset="100%" stop-color="#ce8fed"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgWav)"/><g transform="translate(4,6)" opacity="0.22"><path d="M0,85 Q40,55 80,85 T160,85 T200,85 V200 H0 Z" fill="#1a1a1a"/></g><path d="M0,85 Q40,55 80,85 T160,85 T200,85 V200 H0 Z" fill="url(#wv1)"/><path d="M0,118 Q40,92 80,118 T160,118 T200,118 V200 H0 Z" fill="url(#wv2)"/><path d="M0,150 Q40,128 80,150 T160,150 T200,150 V200 H0 Z" fill="url(#wv3)"/><ellipse cx="60" cy="55" rx="45" ry="12" fill="#ffffff" opacity="0.3" transform="rotate(-15 60 55)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgMtn" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#fddfd1"/><stop offset="100%" stop-color="#edaf91"/></radialGradient><linearGradient id="sunG" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#bae8ff"/><stop offset="100%" stop-color="#69ccff"/></linearGradient><linearGradient id="peak1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#cbac90"/><stop offset="100%" stop-color="#896b51"/></linearGradient><linearGradient id="peak2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#d8c4b0"/><stop offset="100%" stop-color="#a5896f"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgMtn)"/><g transform="translate(4,6)" opacity="0.22"><circle cx="150" cy="52" r="20" fill="#1a1a1a"/></g><g transform="translate(4,6)" opacity="0.22"><path d="M15,155 L78,65 L118,112 L155,50 L188,155 Z" fill="#1a1a1a"/></g><circle cx="150" cy="52" r="20" fill="url(#sunG)"/><path d="M15,155 L78,65 L118,112 L155,50 L188,155 Z" fill="url(#peak2)"/><path d="M155,50 L172,78 L138,78 Z" fill="#ffffff"/><path d="M15,155 L78,65 L108,105 L55,155 Z" fill="url(#peak1)"/><path d="M78,65 L96,92 L60,92 Z" fill="#ffffff"/><ellipse cx="95" cy="60" rx="40" ry="14" fill="#ffffff" opacity="0.25" transform="rotate(-25 95 60)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgRiv" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#fff2f9"/><stop offset="100%" stop-color="#ffd7e9"/></radialGradient><linearGradient id="w1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#da6a85"/><stop offset="100%" stop-color="#9d4a5e"/></linearGradient><linearGradient id="w2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#fe91af"/><stop offset="100%" stop-color="#da6a85"/></linearGradient><linearGradient id="w3" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#ffc9df"/><stop offset="100%" stop-color="#ffa1c3"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgRiv)"/><g transform="translate(4,6)" opacity="0.22"><path d="M0,75 Q40,50 80,75 T160,75 T200,75 V102 Q150,80 100,102 T0,102 Z" fill="#1a1a1a"/></g><path d="M0,75 Q40,50 80,75 T160,75 T200,75 V102 Q150,80 100,102 T0,102 Z" fill="url(#w1)"/><path d="M0,112 Q40,90 80,112 T160,112 T200,112 V138 Q150,116 100,138 T0,138 Z" fill="url(#w2)"/><path d="M0,148 Q40,128 80,148 T160,148 T200,148 V172 Q150,150 100,172 T0,172 Z" fill="url(#w3)"/><ellipse cx="60" cy="60" rx="45" ry="10" fill="#ffffff" opacity="0.3" transform="rotate(-15 60 60)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgDrg" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#e8efdd"/><stop offset="100%" stop-color="#c2d1a2"/></radialGradient><linearGradient id="drgBody" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#97a252"/><stop offset="100%" stop-color="#5b6704"/></linearGradient><linearGradient id="drgWing" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#b4bf7e"/><stop offset="100%" stop-color="#7e8521"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgDrg)"/><g transform="translate(4,6)" opacity="0.22"><path d="M60,150 Q40,120 55,95 Q45,80 60,72 Q58,55 78,52 Q80,35 100,42 Q112,25 130,38 Q150,30 155,50 Q170,52 165,70 Q180,75 170,92 Q178,105 160,108 L165,130 L145,118 L142,140 L122,125 L115,148 L98,128 L82,148 L75,125 Z" fill="#1a1a1a"/></g><path d="M60,150 Q40,120 55,95 Q45,80 60,72 Q58,55 78,52 Q80,35 100,42 Q112,25 130,38 Q150,30 155,50 Q170,52 165,70 Q180,75 170,92 Q178,105 160,108 L165,130 L145,118 L142,140 L122,125 L115,148 L98,128 L82,148 L75,125 Z" fill="url(#drgBody)"/><path d="M130,38 Q145,15 165,25 Q155,40 155,50 Z" fill="url(#drgWing)"/><path d="M100,42 Q108,20 125,28 Q118,42 112,50 Z" fill="url(#drgWing)"/><circle cx="152" cy="58" r="4.5" fill="#000000"/><circle cx="150.5" cy="56" r="1.5" fill="#ffffff"/><path d="M168,68 Q180,66 178,78 Q170,76 168,68 Z" fill="#63b6fa"/><ellipse cx="95" cy="55" rx="40" ry="12" fill="#ffffff" opacity="0.25" transform="rotate(-20 95 55)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgSpc" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#4b3508"/><stop offset="100%" stop-color="#251a04"/></radialGradient><radialGradient id="moonG" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#eef8ff"/><stop offset="100%" stop-color="#bed7f6"/></radialGradient><radialGradient id="planetG" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#90d2ff"/><stop offset="100%" stop-color="#4fa0ef"/></radialGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgSpc)"/><g transform="translate(4,6)" opacity="0.22"><circle cx="150" cy="58" r="26" fill="#000000"/></g><circle cx="150" cy="58" r="26" fill="url(#moonG)"/><circle cx="140" cy="50" r="22" fill="#372701"/><circle cx="45" cy="45" r="2.5" fill="#ffffff"/><circle cx="72" cy="88" r="2" fill="#ffffff"/><circle cx="28" cy="105" r="2.2" fill="#ffffff"/><circle cx="105" cy="135" r="1.8" fill="#ffffff"/><circle cx="165" cy="145" r="2.2" fill="#ffffff"/><ellipse cx="58" cy="158" rx="62" ry="18" fill="none" stroke="#8ac3ff" stroke-width="2.5" opacity="0.6"/><g transform="translate(4,6)" opacity="0.22"><circle cx="58" cy="158" r="13" fill="#000000"/></g><circle cx="58" cy="158" r="13" fill="url(#planetG)"/><ellipse cx="148" cy="50" rx="10" ry="6" fill="#ffffff" opacity="0.4" transform="rotate(-20 148 50)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgFlm" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#e2f5ff"/><stop offset="100%" stop-color="#b8e7ff"/></radialGradient><linearGradient id="flmOut" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#2db7eb"/><stop offset="100%" stop-color="#00869e"/></linearGradient><linearGradient id="flmIn" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#95dcff"/><stop offset="100%" stop-color="#48c1ff"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgFlm)"/><g transform="translate(4,6)" opacity="0.22"><path d="M100,172 C58,172 52,128 74,96 C80,118 96,118 90,90 C112,102 132,128 120,150 C142,138 140,106 124,86 C150,102 156,150 130,166 C162,154 150,106 133,80 C118,54 88,54 88,86 C72,74 68,44 90,24 C58,46 36,88 53,128 C36,140 40,168 100,172 Z" fill="#1a1a1a"/></g><path d="M100,172 C58,172 52,128 74,96 C80,118 96,118 90,90 C112,102 132,128 120,150 C142,138 140,106 124,86 C150,102 156,150 130,166 C162,154 150,106 133,80 C118,54 88,54 88,86 C72,74 68,44 90,24 C58,46 36,88 53,128 C36,140 40,168 100,172 Z" fill="url(#flmOut)"/><path d="M96,160 C79,154 76,132 90,114 C92,128 101,128 97,112 C112,120 121,138 109,152 C124,144 121,120 109,106 C121,116 126,142 109,154 C96,160 96,160 96,160 Z" fill="url(#flmIn)"/><ellipse cx="85" cy="60" rx="20" ry="10" fill="#ffffff" opacity="0.3" transform="rotate(-20 85 60)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgTree" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#fbf0ff"/><stop offset="100%" stop-color="#f5d7ff"/></radialGradient><linearGradient id="tr1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#d382c9"/><stop offset="100%" stop-color="#98548e"/></linearGradient><linearGradient id="tr2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#e294d8"/><stop offset="100%" stop-color="#be6bb3"/></linearGradient><linearGradient id="tr3" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#efa6e9"/><stop offset="100%" stop-color="#d37dca"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgTree)"/><rect x="93" y="150" width="14" height="32" rx="4" fill="#587a87"/><g transform="translate(4,6)" opacity="0.22"><path d="M100,42 L138,105 H62 Z" fill="#1a1a1a"/></g><path d="M100,42 L138,105 H62 Z" fill="url(#tr1)"/><path d="M100,74 L143,140 H57 Z" fill="url(#tr2)"/><path d="M100,110 L148,178 H52 Z" fill="url(#tr3)"/><ellipse cx="95" cy="90" rx="30" ry="10" fill="#ffffff" opacity="0.25" transform="rotate(-25 95 90)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgCry" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#e9efdd"/><stop offset="100%" stop-color="#c3d3a6"/></radialGradient><linearGradient id="cA" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#c4cda6"/><stop offset="100%" stop-color="#a3ad70"/></linearGradient><linearGradient id="cB" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#86934b"/><stop offset="100%" stop-color="#5b6704"/></linearGradient><linearGradient id="cC" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#acb97b"/><stop offset="100%" stop-color="#7e8521"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgCry)"/><g transform="translate(4,6)" opacity="0.22"><path d="M100,28 L148,76 L120,172 L80,172 L52,76 Z" fill="#1a1a1a"/></g><path d="M100,28 L148,76 L120,172 L80,172 L52,76 Z" fill="url(#cA)"/><path d="M100,28 L148,76 L100,92 Z" fill="#dee3ca"/><path d="M100,28 L52,76 L100,92 Z" fill="url(#cB)"/><path d="M52,76 L100,92 L80,172 Z" fill="url(#cB)"/><path d="M148,76 L100,92 L120,172 Z" fill="url(#cC)"/><ellipse cx="95" cy="55" rx="18" ry="22" fill="#ffffff" opacity="0.4" transform="rotate(-10 95 55)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgMoon" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#3c2d07"/><stop offset="100%" stop-color="#231900"/></radialGradient><linearGradient id="moonG2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#dcf1ff"/><stop offset="100%" stop-color="#afd4ff"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgMoon)"/><g transform="translate(4,6)" opacity="0.22"><path d="M132,38 A56,56 0 1 0 132,162 A46,46 0 1 1 132,38 Z" fill="#000000"/></g><path d="M132,38 A56,56 0 1 0 132,162 A46,46 0 1 1 132,38 Z" fill="url(#moonG2)"/><circle cx="56" cy="55" r="2.5" fill="#ffffff"/><circle cx="40" cy="92" r="2" fill="#ffffff"/><circle cx="62" cy="132" r="2.5" fill="#ffffff"/><circle cx="34" cy="152" r="1.8" fill="#ffffff"/><circle cx="90" cy="45" r="1.5" fill="#ffffff"/><ellipse cx="120" cy="55" rx="20" ry="12" fill="#ffffff" opacity="0.3" transform="rotate(-20 120 55)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgCmp" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#ecf7ff"/><stop offset="100%" stop-color="#cee7ff"/></radialGradient><linearGradient id="cmpN" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#189b8c"/><stop offset="100%" stop-color="#006f5e"/></linearGradient><linearGradient id="cmpS" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#bea287"/><stop offset="100%" stop-color="#98785f"/></linearGradient><radialGradient id="cmpRing" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#9dceff"/><stop offset="100%" stop-color="#5999de"/></radialGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgCmp)"/><g transform="translate(4,6)" opacity="0.22"><circle cx="100" cy="100" r="68" fill="none" stroke="#1a1a1a" stroke-width="6"/></g><circle cx="100" cy="100" r="68" fill="none" stroke="url(#cmpRing)" stroke-width="6"/><circle cx="100" cy="100" r="58" fill="#f4fbff" opacity="0.5"/><path d="M100,42 L114,95 L100,112 L86,95 Z" fill="url(#cmpN)"/><path d="M100,158 L114,105 L100,88 L86,105 Z" fill="url(#cmpS)"/><path d="M42,100 L95,86 L112,100 L95,114 Z" fill="url(#cmpS)"/><path d="M158,100 L105,86 L88,100 L105,114 Z" fill="url(#cmpS)"/><circle cx="100" cy="100" r="7" fill="#72acf4"/><ellipse cx="75" cy="65" rx="30" ry="12" fill="#ffffff" opacity="0.35" transform="rotate(-20 75 65)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgWav" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#ffede9"/><stop offset="100%" stop-color="#ffd6ce"/></radialGradient><linearGradient id="wv1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#bb5f46"/><stop offset="100%" stop-color="#8b442d"/></linearGradient><linearGradient id="wv2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#db7c65"/><stop offset="100%" stop-color="#bb5f46"/></linearGradient><linearGradient id="wv3" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#feb5ab"/><stop offset="100%" stop-color="#f1907f"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgWav)"/><g transform="translate(4,6)" opacity="0.22"><path d="M0,85 Q40,55 80,85 T160,85 T200,85 V200 H0 Z" fill="#1a1a1a"/></g><path d="M0,85 Q40,55 80,85 T160,85 T200,85 V200 H0 Z" fill="url(#wv1)"/><path d="M0,118 Q40,92 80,118 T160,118 T200,118 V200 H0 Z" fill="url(#wv2)"/><path d="M0,150 Q40,128 80,150 T160,150 T200,150 V200 H0 Z" fill="url(#wv3)"/><ellipse cx="60" cy="55" rx="45" ry="12" fill="#ffffff" opacity="0.3" transform="rotate(-15 60 55)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgMtn" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#d3ecca"/><stop offset="100%" stop-color="#95cb84"/></radialGradient><linearGradient id="sunG" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#ffc7ff"/><stop offset="100%" stop-color="#ff97ff"/></linearGradient><linearGradient id="peak1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#92bd94"/><stop offset="100%" stop-color="#537b54"/></linearGradient><linearGradient id="peak2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#b1cfb3"/><stop offset="100%" stop-color="#729872"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgMtn)"/><g transform="translate(4,6)" opacity="0.22"><circle cx="150" cy="52" r="20" fill="#1a1a1a"/></g><g transform="translate(4,6)" opacity="0.22"><path d="M15,155 L78,65 L118,112 L155,50 L188,155 Z" fill="#1a1a1a"/></g><circle cx="150" cy="52" r="20" fill="url(#sunG)"/><path d="M15,155 L78,65 L118,112 L155,50 L188,155 Z" fill="url(#peak2)"/><path d="M155,50 L172,78 L138,78 Z" fill="#ffffff"/><path d="M15,155 L78,65 L108,105 L55,155 Z" fill="url(#peak1)"/><path d="M78,65 L96,92 L60,92 Z" fill="#ffffff"/><ellipse cx="95" cy="60" rx="40" ry="14" fill="#ffffff" opacity="0.25" transform="rotate(-25 95 60)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgRiv" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#faf7e8"/><stop offset="100%" stop-color="#ece4be"/></radialGradient><linearGradient id="w1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#8d8a2e"/><stop offset="100%" stop-color="#63621d"/></linearGradient><linearGradient id="w2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#b6af57"/><stop offset="100%" stop-color="#8d8a2e"/></linearGradient><linearGradient id="w3" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#e3dba7"/><stop offset="100%" stop-color="#cabf69"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgRiv)"/><g transform="translate(4,6)" opacity="0.22"><path d="M0,75 Q40,50 80,75 T160,75 T200,75 V102 Q150,80 100,102 T0,102 Z" fill="#1a1a1a"/></g><path d="M0,75 Q40,50 80,75 T160,75 T200,75 V102 Q150,80 100,102 T0,102 Z" fill="url(#w1)"/><path d="M0,112 Q40,90 80,112 T160,112 T200,112 V138 Q150,116 100,138 T0,138 Z" fill="url(#w2)"/><path d="M0,148 Q40,128 80,148 T160,148 T200,148 V172 Q150,150 100,172 T0,172 Z" fill="url(#w3)"/><ellipse cx="60" cy="60" rx="45" ry="10" fill="#ffffff" opacity="0.3" transform="rotate(-15 60 60)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgDrg" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#ddf1ef"/><stop offset="100%" stop-color="#a2d6cf"/></radialGradient><linearGradient id="drgBody" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#52af96"/><stop offset="100%" stop-color="#047858"/></linearGradient><linearGradient id="drgWing" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#7ec9b7"/><stop offset="100%" stop-color="#229872"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgDrg)"/><g transform="translate(4,6)" opacity="0.22"><path d="M60,150 Q40,120 55,95 Q45,80 60,72 Q58,55 78,52 Q80,35 100,42 Q112,25 130,38 Q150,30 155,50 Q170,52 165,70 Q180,75 170,92 Q178,105 160,108 L165,130 L145,118 L142,140 L122,125 L115,148 L98,128 L82,148 L75,125 Z" fill="#1a1a1a"/></g><path d="M60,150 Q40,120 55,95 Q45,80 60,72 Q58,55 78,52 Q80,35 100,42 Q112,25 130,38 Q150,30 155,50 Q170,52 165,70 Q180,75 170,92 Q178,105 160,108 L165,130 L145,118 L142,140 L122,125 L115,148 L98,128 L82,148 L75,125 Z" fill="url(#drgBody)"/><path d="M130,38 Q145,15 165,25 Q155,40 155,50 Z" fill="url(#drgWing)"/><path d="M100,42 Q108,20 125,28 Q118,42 112,50 Z" fill="url(#drgWing)"/><circle cx="152" cy="58" r="4.5" fill="#000000"/><circle cx="150.5" cy="56" r="1.5" fill="#ffffff"/><path d="M168,68 Q180,66 178,78 Q170,76 168,68 Z" fill="#f48bf6"/><ellipse cx="95" cy="55" rx="40" ry="12" fill="#ffffff" opacity="0.25" transform="rotate(-20 95 55)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgSpc" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#0a461e"/><stop offset="100%" stop-color="#05220e"/></radialGradient><radialGradient id="moonG" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#fff2ff"/><stop offset="100%" stop-color="#f4c8ec"/></radialGradient><radialGradient id="planetG" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#ffabff"/><stop offset="100%" stop-color="#e974e1"/></radialGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgSpc)"/><g transform="translate(4,6)" opacity="0.22"><circle cx="150" cy="58" r="26" fill="#000000"/></g><circle cx="150" cy="58" r="26" fill="url(#moonG)"/><circle cx="140" cy="50" r="22" fill="#033415"/><circle cx="45" cy="45" r="2.5" fill="#ffffff"/><circle cx="72" cy="88" r="2" fill="#ffffff"/><circle cx="28" cy="105" r="2.2" fill="#ffffff"/><circle cx="105" cy="135" r="1.8" fill="#ffffff"/><circle cx="165" cy="145" r="2.2" fill="#ffffff"/><ellipse cx="58" cy="158" rx="62" ry="18" fill="none" stroke="#ffa1f4" stroke-width="2.5" opacity="0.6"/><g transform="translate(4,6)" opacity="0.22"><circle cx="58" cy="158" r="13" fill="#000000"/></g><circle cx="58" cy="158" r="13" fill="url(#planetG)"/><ellipse cx="148" cy="50" rx="10" ry="6" fill="#ffffff" opacity="0.4" transform="rotate(-20 148 50)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgFlm" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#ffecff"/><stop offset="100%" stop-color="#fcd2ff"/></radialGradient><linearGradient id="flmOut" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#e17dff"/><stop offset="100%" stop-color="#944fe3"/></linearGradient><linearGradient id="flmIn" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#ffb0ff"/><stop offset="100%" stop-color="#ff88ff"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgFlm)"/><g transform="translate(4,6)" opacity="0.22"><path d="M100,172 C58,172 52,128 74,96 C80,118 96,118 90,90 C112,102 132,128 120,150 C142,138 140,106 124,86 C150,102 156,150 130,166 C162,154 150,106 133,80 C118,54 88,54 88,86 C72,74 68,44 90,24 C58,46 36,88 53,128 C36,140 40,168 100,172 Z" fill="#1a1a1a"/></g><path d="M100,172 C58,172 52,128 74,96 C80,118 96,118 90,90 C112,102 132,128 120,150 C142,138 140,106 124,86 C150,102 156,150 130,166 C162,154 150,106 133,80 C118,54 88,54 88,86 C72,74 68,44 90,24 C58,46 36,88 53,128 C36,140 40,168 100,172 Z" fill="url(#flmOut)"/><path d="M96,160 C79,154 76,132 90,114 C92,128 101,128 97,112 C112,120 121,138 109,152 C124,144 121,120 109,106 C121,116 126,142 109,154 C96,160 96,160 96,160 Z" fill="url(#flmIn)"/><ellipse cx="85" cy="60" rx="20" ry="10" fill="#ffffff" opacity="0.3" transform="rotate(-20 85 60)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgTree" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#fff0ee"/><stop offset="100%" stop-color="#ffd7d0"/></radialGradient><linearGradient id="tr1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#ce8e63"/><stop offset="100%" stop-color="#925e39"/></linearGradient><linearGradient id="tr2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#dd9f75"/><stop offset="100%" stop-color="#b8784b"/></linearGradient><linearGradient id="tr3" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#edb08a"/><stop offset="100%" stop-color="#cf8a5c"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgTree)"/><rect x="93" y="150" width="14" height="32" rx="4" fill="#846c90"/><g transform="translate(4,6)" opacity="0.22"><path d="M100,42 L138,105 H62 Z" fill="#1a1a1a"/></g><path d="M100,42 L138,105 H62 Z" fill="url(#tr1)"/><path d="M100,74 L143,140 H57 Z" fill="url(#tr2)"/><path d="M100,110 L148,178 H52 Z" fill="url(#tr3)"/><ellipse cx="95" cy="90" rx="30" ry="10" fill="#ffffff" opacity="0.25" transform="rotate(-25 95 90)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgCry" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#dcf1ee"/><stop offset="100%" stop-color="#a6d7d2"/></radialGradient><linearGradient id="cA" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#a5d3ca"/><stop offset="100%" stop-color="#70b6a5"/></linearGradient><linearGradient id="cB" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#4b9e8a"/><stop offset="100%" stop-color="#047858"/></linearGradient><linearGradient id="cC" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#7bc2b2"/><stop offset="100%" stop-color="#229872"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgCry)"/><g transform="translate(4,6)" opacity="0.22"><path d="M100,28 L148,76 L120,172 L80,172 L52,76 Z" fill="#1a1a1a"/></g><path d="M100,28 L148,76 L120,172 L80,172 L52,76 Z" fill="url(#cA)"/><path d="M100,28 L148,76 L100,92 Z" fill="#cae6e0"/><path d="M100,28 L52,76 L100,92 Z" fill="url(#cB)"/><path d="M52,76 L100,92 L80,172 Z" fill="url(#cB)"/><path d="M148,76 L100,92 L120,172 Z" fill="url(#cC)"/><ellipse cx="95" cy="55" rx="18" ry="22" fill="#ffffff" opacity="0.4" transform="rotate(-10 95 55)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgMoon" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#093b1c"/><stop offset="100%" stop-color="#01220d"/></radialGradient><linearGradient id="moonG2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#ffe3ff"/><stop offset="100%" stop-color="#ffbdf5"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgMoon)"/><g transform="translate(4,6)" opacity="0.22"><path d="M132,38 A56,56 0 1 0 132,162 A46,46 0 1 1 132,38 Z" fill="#000000"/></g><path d="M132,38 A56,56 0 1 0 132,162 A46,46 0 1 1 132,38 Z" fill="url(#moonG2)"/><circle cx="56" cy="55" r="2.5" fill="#ffffff"/><circle cx="40" cy="92" r="2" fill="#ffffff"/><circle cx="62" cy="132" r="2.5" fill="#ffffff"/><circle cx="34" cy="152" r="1.8" fill="#ffffff"/><circle cx="90" cy="45" r="1.5" fill="#ffffff"/><ellipse cx="120" cy="55" rx="20" ry="12" fill="#ffffff" opacity="0.3" transform="rotate(-20 120 55)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgCmp" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#fff0ff"/><stop offset="100%" stop-color="#ffd6fe"/></radialGradient><linearGradient id="cmpN" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#8372e5"/><stop offset="100%" stop-color="#5443bd"/></linearGradient><linearGradient id="cmpS" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#89b18c"/><stop offset="100%" stop-color="#62885f"/></linearGradient><radialGradient id="cmpRing" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#ffaefa"/><stop offset="100%" stop-color="#d975ce"/></radialGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgCmp)"/><g transform="translate(4,6)" opacity="0.22"><circle cx="100" cy="100" r="68" fill="none" stroke="#1a1a1a" stroke-width="6"/></g><circle cx="100" cy="100" r="68" fill="none" stroke="url(#cmpRing)" stroke-width="6"/><circle cx="100" cy="100" r="58" fill="#fff7ff" opacity="0.5"/><path d="M100,42 L114,95 L100,112 L86,95 Z" fill="url(#cmpN)"/><path d="M100,158 L114,105 L100,88 L86,105 Z" fill="url(#cmpS)"/><path d="M42,100 L95,86 L112,100 L95,114 Z" fill="url(#cmpS)"/><path d="M158,100 L105,86 L88,100 L105,114 Z" fill="url(#cmpS)"/><circle cx="100" cy="100" r="7" fill="#f089de"/><ellipse cx="75" cy="65" rx="30" ry="12" fill="#ffffff" opacity="0.35" transform="rotate(-20 75 65)"/></svg>',
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><radialGradient id="bgWav" cx="35%" cy="30%" r="75%"><stop offset="0%" stop-color="#ebf4e1"/><stop offset="100%" stop-color="#d2e6ba"/></radialGradient><linearGradient id="wv1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#4d8423"/><stop offset="100%" stop-color="#326115"/></linearGradient><linearGradient id="wv2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#6ca03e"/><stop offset="100%" stop-color="#4d8423"/></linearGradient><linearGradient id="wv3" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#b0d088"/><stop offset="100%" stop-color="#86b453"/></linearGradient></defs><rect width="200" height="200" rx="28" fill="url(#bgWav)"/><g transform="translate(4,6)" opacity="0.22"><path d="M0,85 Q40,55 80,85 T160,85 T200,85 V200 H0 Z" fill="#1a1a1a"/></g><path d="M0,85 Q40,55 80,85 T160,85 T200,85 V200 H0 Z" fill="url(#wv1)"/><path d="M0,118 Q40,92 80,118 T160,118 T200,118 V200 H0 Z" fill="url(#wv2)"/><path d="M0,150 Q40,128 80,150 T160,150 T200,150 V200 H0 Z" fill="url(#wv3)"/><ellipse cx="60" cy="55" rx="45" ry="12" fill="#ffffff" opacity="0.3" transform="rotate(-15 60 55)"/></svg>'
];
  v_idx int := ((p_seed - 1) % 40) + 1;
begin
  return 'data:image/svg+xml;base64,' || replace(encode(convert_to(v_svgs[v_idx], 'UTF8'), 'base64'), chr(10), '');
end;
$BODY$;
...
-- registration path. display_name is picked in order from a fixed roster
-- of real-looking names (v_captain_names / v_player_names below) instead
-- of a generic "临时队长N" / "临时队员N" label, so temp accounts read
-- naturally in the lobby, draft pools, and Final Matchups poster during
-- testing/demos; falls back to the old numbered pattern only if a
-- tournament calls for more temp accounts than a list has entries for.
-- avatar_url is likewise set for every temp account (never left null):
-- a small original "badge icon" SVG built by _temp_avatar_svg() above
-- (see that function's comment), embedded directly as a data URI -- no
-- network call, no external image host of any kind. Every one of the 8
-- captains and every one of the 32 players gets its own distinct
-- theme + colorway combination -- same idea as the display names, just
-- for the avatar shown next to them everywhere avatarUrl is rendered.
...
declare
  ...
  v_name      text;
  v_avatar    text;
  -- Runs once per account across BOTH loops (1 for the first captain,
  -- ..., 40 for the last player by default) so every avatar_url built
  -- from it below is guaranteed unique across all captains AND players,
  -- not just within one role.
  v_global_i  int := 0;
  -- Fixed display names for temp captains/players, in order. Falls back
  -- to the old '临时队长N' / '临时队员N' pattern past the end of each list
  -- so this still works for tournaments larger than 8 teams / 5 players
  -- per team (the default shape these lists were sized for: 8 captains,
  -- 8 x (5-1) = 32 players).
  v_captain_names constant text[] := array[
    '龚建ZSMJ', 'Ok林仔', '月夜枫', '黄翔LongDD', '谢斌DD', 'Zhou陈尧', '刘嘉俊Sylar1', '保护我方蛋饼'
  ];
  v_player_names constant text[] := array[
    '小菡菡', 'sed', '林九鸽', '小胡', '一只蘇', '三酒', '骇客', '徐杰',
    '艾琳', '顾非池', '宝哥', '奥特馒', '狗哥', '阿雅', '朝朝', '目黑川',
    '石页', '小琳达', '小300', '1con', '小蝴蝶', 'doinb', '果小果是个弟弟', '甜瓜',
    '培莉娅', '白仔', '煊宝', '炸毛张', '蛋糕', '憨整', '阿绿', '南枫'
  ];
begin
  perform public._require_role(p_token, array['admin', 'developer']);
  ...
  for v_i in 1..coalesce(p_captain_count, 0) loop
    v_global_i := v_global_i + 1;
    v_gender := case when v_i % 2 = 0 then 'male' else 'female' end;
    v_name := case when v_i <= array_length(v_captain_names, 1) then v_captain_names[v_i] else '临时队长' || v_i end;
    -- Theme + colorway picked from v_global_i, see _temp_avatar_svg() above.
    v_avatar := public._temp_avatar_svg(v_global_i);

    insert into public.accounts (username, display_name, tournament_role, gender, permission_role, is_temp, avatar_url)
    values (
      'temp_' || substr(replace(gen_random_uuid()::text, '-', ''), 1, 12),
      v_name,
      'captain',
      v_gender,
      'user',
  ...
  for v_i in 1..coalesce(p_player_count, 0) loop
    v_global_i := v_global_i + 1;
    v_gender := case when v_i % 2 = 0 then 'male' else 'female' end;
    v_name := case when v_i <= array_length(v_player_names, 1) then v_player_names[v_i] else '临时队员' || v_i end;
    v_avatar := public._temp_avatar_svg(v_global_i);
...
-- Random Roll, optionally scoped to an explicit "pool" of teams (Phase 5's
-- 01 冠军海报版 stage: the admin can select any number of not-yet-matched
-- teams from the casting pool -- no 2-team cap -- and only those teams
-- are shuffled). The pool is OPTIONAL: if p_team_idxs is null or empty,
-- this rolls every currently-free team instead (computed here, server-
-- side, at execution time -- not guessed by the client), so clicking
-- Random Roll with nothing selected still works exactly like the old
-- "roll everyone" behavior. Either way, this NEVER touches anything
-- already in the matchups array, locked or unlocked, and never touches
-- any free team that wasn't part of whatever pool (explicit or default)
-- ends up being rolled. New pairs are appended after whatever's already
-- there, same shuffle-then-pair algorithm as roll_tournament_matchups():
-- odd pool size means the last team drawn gets a bye (b = null) rather
-- than being left out entirely.
-- Examples: pool of 2 -> 1 matchup. Pool of 3 -> 1 matchup + 1 bye. Pool
-- of 5 -> 2 matchups + 1 bye. Pool of 7 -> 3 matchups + 1 bye. Empty pool
-- with 8 free teams -> 4 matchups, same as rolling everyone. And so on
-- for any pool size.
create or replace function public.roll_tournament_matchups_pool(
  p_token      uuid,
  p_team_idxs  integer[] default null
)
returns public.tournament_matches
language plpgsql
security definer
...
begin
  ...
  -- every team currently in ANY existing matchup (locked or unlocked) --
  -- needed either way: to validate an explicit pool, or to compute the
  -- default "everyone free" pool.
  for v_m in select * from jsonb_array_elements(v_row.matchups)
  loop
    if v_m->>'a' is not null then v_used_idxs := v_used_idxs || (v_m->>'a')::int; end if;
    if v_m->>'b' is not null then v_used_idxs := v_used_idxs || (v_m->>'b')::int; end if;
  ...
  v_explicit := p_team_idxs is not null and array_length(p_team_idxs, 1) is not null and array_length(p_team_idxs, 1) >= 1;

  if v_explicit then
    -- every idx must be a real team, and no duplicates within the pool itself
    foreach v_idx in array p_team_idxs
    loop
      if v_idx is null or not (v_idx = any(v_valid_idxs)) then
        raise exception 'invalid_team_index' using errcode = '22000';
      end if;
    ...
    -- none of the pooled teams may already belong to ANY existing matchup
    -- (locked or unlocked) -- same rule create_manual_matchup() enforces,
    -- since a team can only ever be in one matchup at a time.
    foreach v_idx in array p_team_idxs
    loop
      if v_idx = any(v_used_idxs) then
        raise exception 'team_already_matched' using errcode = '22000';
      end if;
    end loop;
    ...
    -- No pool given -> default to every currently-free team, computed
    -- here from this row's own current state, not from anything the
    -- client guessed. If nothing is free, v_pool ends up empty and the
    -- loop below simply does nothing -- a harmless no-op, not an error.
    select coalesce(array_agg(x), '{}') into v_pool
    from unnest(v_valid_idxs) as x
    where not (x = any(v_used_idxs));
  end if;

  v_pool_count := coalesce(array_length(v_pool, 1), 0);

  if v_pool_count > 0 then
    select array_agg(x order by random()) into v_shuffled from unnest(v_pool) as x;

    while v_ptr <= v_pool_count loop
      v_a := v_shuffled[v_ptr]; v_ptr := v_ptr + 1;
      if v_ptr <= v_pool_count then v_b := v_shuffled[v_ptr]; v_ptr := v_ptr + 1; else v_b := null; end if;
      v_rolled := v_rolled || jsonb_build_array(jsonb_build_object('a', v_a, 'b', v_b, 'locked', false));
    end loop;
  end if;

  update public.tournament_matches
  set matchups   = v_row.matchups || v_rolled,
      updated_at = now(),
      updated_by = v_actor.id
  where id = true
  returning * into v_row;
...
declare
  v_actor public.accounts;
  v_row   public.tournament_draft_state;
begin
  v_actor := public._require_role(p_token, array['admin', 'developer']);
...
grant execute on function
  ...
  public.roll_tournament_matchups_pool(uuid, integer[]),
